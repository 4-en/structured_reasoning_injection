...
N_TRAINING_RUNS_PER_EVAL = [0, 1, 10]
N_EVAL_GOLDENS = 11
N_TRAINING_GOLDENS = 50
USE_CACHE = True
...
llm = DynMemLLM(model_name=model_name, pipeline=pipeline)
...
all_goldens = single_turn_benchmark.goldens

# split randomly into eval and training sets
random.seed(42)
random.shuffle(all_goldens)
eval_goldens = all_goldens[:N_EVAL_GOLDENS]
training_goldens = all_goldens[N_EVAL_GOLDENS:N_EVAL_GOLDENS+N_TRAINING_GOLDENS]
single_turn_benchmark.goldens = eval_goldens
if len(training_goldens) < N_TRAINING_GOLDENS:
    print(f"Warning: Not enough goldens to create training set of {N_TRAINING_GOLDENS}. Only found {len(training_goldens)}.")
...
# set iterations to low value for better learning contrast
pipeline.config.max_retrieval_iterations = 2

for n_training_runs in N_TRAINING_RUNS_PER_EVAL:
    while training_run_count < n_training_runs:
        print(f"\n=== Training Run {training_run_count+1} of {n_training_runs} ===")
        pipeline.config.enable_dynamic_learning = True
        # shuffle training goldens each run
        random.shuffle(training_goldens)
        for i, golden in enumerate(training_goldens):
            output = llm.generate(golden.input)
            print(f"[{i}/{len(training_goldens)}] Q: {golden.input}\nA: {output}")
            
        training_run_count += 1
    
    print(f"\n=== Evaluation after {n_training_runs} training runs ===")
    # disable learning in config
    pipeline.config.enable_dynamic_learning = False
    single_turn_results = single_turn_benchmark.run(llms=[llm])
    
    # dict should only have one key since we passed a single LLM
    single_result = list(single_turn_results.values()).pop()
    assert single_result is not None, "No results found for single LLM evaluation."
    
    all_results[f"DynMem_{n_training_runs}_runs"] = single_result
    
    
    pipeline.config.enable_dynamic_learning = True  # reset for next training
...
for model_name, results in all_results.items():
    ...
    for avg_name, avg_score in avg_scores.items():
        all_results[model_name][avg_name] = avg_score
...
output_file = f"learning_benchmark_{timestamp}.json"
